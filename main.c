...
void *get_page(Pager *pager, uint32_t page_num) {
  ...
  // Cache miss condition. Here we allocate memory and load from file
  if (pager->pages[page_num] == NULL) {
    void *page = malloc(PAGE_SIZE);
    uint32_t num_pages = pager->file_length / PAGE_SIZE;

    // We might save a partial page at eof
...
Pager *pager_open(const char *filename) {
  ...
  // gets file length. Returns (0 + size of file)
 ...
 * -> initialize a pager ds
 * -> initialize a table ds
 */
...
void pager_flush(Pager *pager, uint32_t page_num, uint32_t size) {
  ...
  // set file pointer to start of copying line
 ...
 * 3. Frees memory for pager and table ds
 */
void db_close(Table *table) {
  ...
  // There may be a partial page to wrote to eof
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    case (PREPARE_SYNTAX_ERROR):
      printf("Syntax error at start of '%s'. Could not parse statement.\n",
             input_buffer->buffer);
      continue;
